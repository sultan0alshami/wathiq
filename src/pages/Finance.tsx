...
export const Finance: React.FC = () => {
  ...
  const titleValidation = useMemo(() => 
    validateField(newEntryTitle, [
      ValidationRules.required(ARABIC_FINANCE_MESSAGES.TRANSACTION_TITLE_REQUIRED),
      ValidationRules.minLength(3, ARABIC_FINANCE_MESSAGES.TRANSACTION_TITLE_MIN_LENGTH),
      ValidationRules.arabicText()
    ]), [newEntryTitle]);

  const amountValidation = useMemo(() =>
    validateField(newEntryAmount, [
      ValidationRules.required(ARABIC_FINANCE_MESSAGES.AMOUNT_REQUIRED),
      ValidationRules.positiveNumber(ARABIC_FINANCE_MESSAGES.AMOUNT_POSITIVE_NUMBER)
    ]), [newEntryAmount]);

  const categoryValidation = useMemo(() =>
    validateField(newEntryCategory, [
      ValidationRules.required(ARABIC_FINANCE_MESSAGES.CATEGORY_REQUIRED),
      ValidationRules.minLength(2, ARABIC_FINANCE_MESSAGES.CATEGORY_MIN_LENGTH),
      ValidationRules.arabicText()
    ]), [newEntryCategory]);
  ...
  const addEntry = useCallback(() => {
    ...
    if (!isFormValid) {
      toast({
        title: ARABIC_FINANCE_MESSAGES.FORM_ERROR_TITLE,
        description: ARABIC_FINANCE_MESSAGES.FORM_ERROR_DESCRIPTION,
        variant: "destructive",
      });
      return;
    }
  ...
  const removeEntry = useCallback(async (id: string) => {
    try {
      ...
      if (entryToDelete) {
        toast({
          title: ARABIC_FINANCE_MESSAGES.TRANSACTION_DELETED_TITLE,
          description: ARABIC_FINANCE_MESSAGES.TRANSACTION_DELETED_DESCRIPTION(entryToDelete.title),
        });
      }
    } catch (err) {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold text-primary">{ARABIC_FINANCE_MESSAGES.FINANCE_TITLE}</h1>
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-primary">{ARABIC_FINANCE_MESSAGES.PAGE_TITLE}</h1>
        <Badge variant="outline" className="text-lg px-4 py-2">
          {formatDate(currentDate, 'dd/MM/yyyy')}
        </Badge>
      </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <SearchInput
                ...
                placeholder={ARABIC_FINANCE_MESSAGES.SEARCH_PLACEHOLDER}
                onClear={clearSearch}
                aria-label={ARABIC_FINANCE_MESSAGES.SEARCH_PLACEHOLDER}
              />
            </div>
            {hasActiveFilters && (
              <Button onClick={clearSearch} variant="outline" size="sm">
                <Filter className="w-4 h-4 ml-2" />
                {ARABIC_FINANCE_MESSAGES.CLEAR_FILTERS_BUTTON}
              </Button>
            )}
          </div>
        </CardContent>
      ...
      <Card className="border-primary/20">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-primary">
            ...
            {ARABIC_FINANCE_MESSAGES.CURRENT_LIQUIDITY_TITLE}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-4">
            <Label htmlFor="liquidity">{ARABIC_FINANCE_MESSAGES.AMOUNT_LABEL}</Label>
            <Input
              id="liquidity"
              type="text"
              ...
              className="max-w-xs"
              placeholder={ARABIC_FINANCE_MESSAGES.LIQUIDITY_PLACEHOLDER}
            />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-primary">{ARABIC_FINANCE_MESSAGES.ADD_TRANSACTION_TITLE}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label>{ARABIC_FINANCE_MESSAGES.TRANSACTION_TITLE_LABEL}</Label>
              <Input
                placeholder={ARABIC_FINANCE_MESSAGES.TRANSACTION_TITLE_PLACEHOLDER}
                value={newEntryTitle}
            ...
            <div className="space-y-2">
              <Label>{ARABIC_FINANCE_MESSAGES.TYPE_LABEL}</Label>
              <Select value={newEntryType} onValueChange={(value: 'income' | 'expense' | 'deposit') => setNewEntryType(value)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="income">{ARABIC_FINANCE_MESSAGES.TYPE_INCOME}</SelectItem>
                  <SelectItem value="expense">{ARABIC_FINANCE_MESSAGES.TYPE_EXPENSE}</SelectItem>
                  <SelectItem value="deposit">{ARABIC_FINANCE_MESSAGES.TYPE_DEPOSIT}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            
            <div className="space-y-2">
              <Label>{ARABIC_FINANCE_MESSAGES.AMOUNT_INPUT_LABEL}</Label>
              <Input
                type="text"
                placeholder={ARABIC_FINANCE_MESSAGES.AMOUNT_INPUT_PLACEHOLDER}
            ...
            <div className="space-y-2">
              <Label>{ARABIC_FINANCE_MESSAGES.CATEGORY_LABEL}</Label>
              <Input
                placeholder={ARABIC_FINANCE_MESSAGES.CATEGORY_PLACEHOLDER}
            ...
            <div className="md:col-span-2 space-y-2">
              <Label>{ARABIC_FINANCE_MESSAGES.ADDITIONAL_DESCRIPTION_LABEL}</Label>
              <Textarea
                placeholder={ARABIC_FINANCE_MESSAGES.ADDITIONAL_DESCRIPTION_PLACEHOLDER}
          ...
          >
            ...
            {ARABIC_FINANCE_MESSAGES.ADD_TRANSACTION_BUTTON}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-primary">{ARABIC_FINANCE_MESSAGES.DAILY_TRANSACTIONS_TITLE}</CardTitle>
        ...
        <CardContent>
          {filteredEntries.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>
                {hasActiveFilters 
                  ? ARABIC_FINANCE_MESSAGES.NO_MATCHING_TRANSACTIONS
                  : ARABIC_FINANCE_MESSAGES.NO_TRANSACTIONS_ADDED
                }
              </p>
            </div>
          ) : (
              ...
              {paginatedEntries.map((entry) => (
                <div key={entry.id} className={`flex items-center justify-between p-4 rounded-lg border ${getEntryColor(entry.type)}`}>
                  ...
                  <Button
                    ...
                    aria-label={`حذف ${ARABIC_FINANCE_MESSAGES.FINANCIAL_TRANSACTION_ITEM_NAME}`}
                    title={`حذف ${ARABIC_FINANCE_MESSAGES.FINANCIAL_TRANSACTION_ITEM_NAME}`}
                  >
      ...
      <DeleteConfirmationDialog
        ...
        itemName={ARABIC_FINANCE_MESSAGES.FINANCIAL_TRANSACTION_ITEM_NAME}
      />

      {/* Save Alert */}
      <Alert>
        <AlertDescription>
          {ARABIC_FINANCE_MESSAGES.SAVE_ALERT_DESCRIPTION}
        </AlertDescription>
      </Alert>
    </div>
  );
