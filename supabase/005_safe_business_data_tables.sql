...
CREATE TABLE IF NOT EXISTS marketing_customer_interactions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    date DATE NOT NULL,
    customer_name VARCHAR(200) NOT NULL,
...
-- Sales indexes
CREATE INDEX IF NOT EXISTS idx_sales_meetings_user_date ON sales_meetings(user_id, date);
CREATE INDEX IF NOT EXISTS idx_sales_meetings_outcome ON sales_meetings(outcome);
...
-- Marketing indexes
CREATE INDEX IF NOT EXISTS idx_marketing_campaigns_user_date ON marketing_campaigns(user_id, date);
...
CREATE INDEX IF NOT EXISTS idx_marketing_interactions_user_date ON marketing_customer_interactions(user_id, date);

-- Customers indexes
CREATE INDEX IF NOT EXISTS idx_customers_user_date ON customers(user_id, date);
